...
#   read data
    ...
    dat1 = pd.read_csv('GOOG_tick.csv', sep=',', index_col=0)
    dat1.head()

#   Step 1.
    print('Step1')
#   VPIN caculating step
    time_bucket_list, bucket_price, vpin_norm_list, high_list, low_list, time_list = vpin_combine(dat1,
                                                                                                  bars_per_bucket=30,
                                                                                                  buckets_per_day=400,
...
#   df_sample_sorted chooses 10000 samples
    ...
    print("\n\n\nMIR list)")
    print(sorted(np.abs(mir_list))[:100])
    print('\n\n\n')
    print(mir_list)
    print('\n\n\n')
